...
if __name__ == '__main__':
    ...
    for i in range(count):
        numbers.append(i // d)
    numbers = keras.utils.to_categorical(numbers, num_classes)
    numbers = tf.cast(numbers, tf.float32)
    noise_and_labels = tf.concat([noise, numbers], 1)
    fake_images = trained_gen.predict(noise_and_labels)
    converted_images = tf.image.resize(fake_images, (96, 96)).numpy()
    save_images(converted_images, 'cond_exp/test1')
